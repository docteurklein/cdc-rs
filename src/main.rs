...
async fn main() -> Result<()> {
    ...
    let (log_pos_tx, mut log_pos_rx) = mpsc::channel::<(i64, String)>(1000);
    ...
    let log_pos_persister = tokio::spawn(async move {
    ...
    let (pubsub_tx, mut pubsub_rx) = mpsc::channel::<(&String, &String, Vec<PubsubMessage>)>(1000);
    ...
    let publisher_task = tokio::spawn(async move {
        ...
        while let Some((db, table, msgs)) = pubsub_rx.recv().await {
             let publisher = publishers.entry(table.clone().into_owned()).or_insert_with(|| {
                 let topic = rhai
    ...
    while let Some(event) = binlog_stream.next() {
        ...
        if let Some(e) = event.read_data()? {
            ...
            if event.header().log_pos() > 0 {
                log_pos_tx.send((event.header().log_pos() as i64, filename.clone())).await?;
            }
        }
    }
    log_pos_persister.await?;
